       ...
       stage('Push Image To Docker Hub') { 
         ...
         //                  sh 'docker run -p 8000:8000 -d nani123456789/$JOB_NAME:latest'
                          ...
                          sshagent(['docker_Server']) {                     
                           ...
                           sh "ssh -o StrictHostKeyChecking=no yash@192.168.80.137 ${dockerrun}"
                       }     
                  }
                }   
            }
             ...
             steps {
                ...
                sh 'docker run --name dast_full --network project_project -t owasp/zap2docker-stable zap-full-scan.py -t http://192.168.80.137/LoginWebApp/ || true'
                sh 'docker run --name dast_baseline --network project_project -t owasp/zap2docker-stable zap-baseline.py -t http://192.168.80.137/LoginWebApp/ --autooff || true'
             }
        }
    }
}
